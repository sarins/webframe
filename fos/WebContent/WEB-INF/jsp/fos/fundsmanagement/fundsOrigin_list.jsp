...
<div class="gridInfo">
    <div class="gridToolbar">
        ...
        <div class="gridPageBtn">
			<huatek:ajaxCutPage pageBean="${pageBean}" formName="sourceUseFunds" offerPageSize="20,50,100" isExistForm="true" queryFunction="queryData();"/>
	</div>
        <div class="clear"></div>
    </div>
    <div class="grid">
        <table width="100%" class="resize" id="tb_funds_Ori_table_list">
            <thead>
                <tr>
				<th>序号</th>
				<th>一级项目分类</th>
            ...
            <tbody>
				...
				<tr <c:if test="${!(s.index%2==0) }"> class="tdbgcolor"</c:if>>
					...
					<td><c:out value="${sourceUseFunds.sencondCategory.proCateName}"/></td>
					...
					<td><c:if test="${sourceUseFunds.isModify == '1'}"><a href="#"  onclick="getData('fundsOrigin.do?actionMethod=editQuery&id=${sourceUseFunds.id}&gatherDate=${sourceUseFunds.gatherDate}','','workspace');">修改</a>
					<c:if test="${superQuery != 'true'}">&nbsp;<a href="#"  onclick="deleteData('${sourceUseFunds.id}');">删除</a>
					&nbsp;<a href="#"  onclick="getData('fundsOrigin.do?actionMethod=editQuery&id=${sourceUseFunds.id}&iscopy=true&gatherDate=${sourceUseFunds.gatherDate}','','workspace');">复制</a>
					</c:if>
					</c:if>
					</td>
			 </tr>
    ...
    <div class="gridToolbar">
        <div class="gridPageBtn">
		<huatek:ajaxCutPage pageBean="${pageBean}" formName="sourceUseFunds" offerPageSize="20,50,100" isOuterForm="true" isExistForm="true" queryFunction="queryData();"/>
		</div>
        <div class="clear"></div>
    </div>
    </div>