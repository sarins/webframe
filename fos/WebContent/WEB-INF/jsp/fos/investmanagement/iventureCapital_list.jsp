...
<div class="gridInfo">
    <div class="gridToolbar">
        <div class="gridHeaderName">创投信息列表</div>
    ...
    <div class="grid">
        <table width="100%" class="resize" id="tb_funds_apply_table_list">
            <thead>
				...
				<th>统计日期</th>
				<th>备注</th>
				<th><c:if test="${superQuery != 'true'}">操作</c:if></th>
			</tr>
            </thead>
            <tbody>
				...
				<tr <c:if test="${!(s.index%2==0) }"> class="tdbgcolor"</c:if>>
					...
					<td>
						<div title="${sourceUseFunds.remarks}"><huatek:out value="${sourceUseFunds.remarks}" size="20" encode="true"/></div>
					</td>
					<td>
					<c:if test="${superQuery != 'true'}">
					<c:if test="${sourceUseFunds.firstCategory.proCateCode == '05'}"><a href="#"  onclick="getData('iventureCapital.do?actionMethod=editQuery&id=${sourceUseFunds.id}&gatherDate=${sourceUseFunds.gatherDate}&subType=subIncome','','workspace');">修改</a></c:if>
					<c:if test="${sourceUseFunds.firstCategory.proCateCode == '06'}"><a href="#"  onclick="getData('iventureCapital.do?actionMethod=editQuery&id=${sourceUseFunds.id}&gatherDate=${sourceUseFunds.gatherDate}&subType=subPay','','workspace');">修改</a></c:if>
					&nbsp;<a href="#"  onclick="deleteData('${sourceUseFunds.id}');">删除</a>
					</c:if>
					</td>
			 </tr>
			</c:forEach>
